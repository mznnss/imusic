/* ============================================================
   ...
   Multi-Engine Synchronized Lyrics
   ============================================================ */
...
function parseTwoRow(r) {
  ...
  if (
    r.thumbnailRenderer &&
    findFirst(r, 'musicThumbnailRenderer')
  ) {
    const style =
      findFirst(r, 'musicThumbnailRenderer')
        .thumbnailCrop;
    ...
    if (
      style === 'MUSIC_THUMBNAIL_CROP_CIRCLE'
    ) {
      item.type = 'artist';
    }
  }
...
function parseListItem(r) {
  ...
  const navInfo =
    endpointInfo(r.navigationEndpoint);
  ...
  const fixed = findFirst(
    r,
    'musicResponsiveListItemFixedColumnRenderer'
  );

  if (fixed) {
    item.duration = normalizeDuration(
      text(fixed.text)
    );
  }
...
/* ============================================================
   ...
   Playback is handled by the YouTube IFrame Player
   in public/app.js.
   ...
   - redirect to temporary YouTube audio URLs

   This avoids the previous:
   "Track loading failed, trying next track..."
   flow caused by direct media extraction.
   ============================================================ */
...
const SEARCH_PARAMS = {
  ...
  playlists:
    'EgeKAQQoAEABagwQDhAKEAMQBBAJEAU=',
};
...
app.get('/api/next', async (req, res) => {
  try {
    ...
    for (const tab of findAll(
      ...
      if (id.startsWith('MPLYt')) {
        lyricsBrowseId = id;
      }

      if (id.startsWith('MPTRt')) {
        relatedBrowseId = id;
      }
    }
...
async function browsePage(
  ...
  if (hResp) {
    header = {
      ...
      subtitle: [
        text(hResp.subtitle),
        text(hResp.secondSubtitle),
      ]
        .filter(Boolean)
        .join(' • '),

      description:
        text(hResp.description) ||
        text(
          findFirst(
            hResp,
            'description'
...
function cleanTitle(t) {
  return String(
    ...
    .replace(
      /[\(\[]\s*(4k|hd|hq|8d( audio)?|1080p|720p)\s*[\)\]]/gi,
      ''
    )
    .replace(
      /\s*-\s*(official|lyric(s)?|audio|video|visualizer|topic).*/gi,
...
async function neteaseLyrics(
  ...
  try {
    ...
    const r =
      await fetchTimeout(
        `https://music.163.com/api/search/get/web?s=${encodeURIComponent(
          q
        )}&type=1&limit=8`,
        {
          headers: {
...
function extractYtmLyrics(
  d
) {
  for (
    const shelf of findAll(
      d,
...
if (
  ...
  app.listen(
    ...
    () => {
      console.log(
        `IMusic running on http://localhost:${PORT}`
      );
    }
  );
}
