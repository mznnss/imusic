/* ============================================================
   iMusic — SPA Frontend
   Streams via YouTube IFrame player, metadata via proxy,
   synced lyrics via LRCLIB, & background playback keep-alive.
   ============================================================ */
...
// 1 detik silent audio base64 WAV agar OS tidak memutus audio task saat layar mati
...
function updateQualityButton() {
  ...
  btn.title = Player.hq ? 'YouTube max quality — tap for standard' : 'Standard audio — tap for max quality';
...
function toggleQuality() {
  ...
  updateQualityButton();
  toast(Player.hq ? 'Max quality' : 'Standard audio');
...
function updateMediaSession(s) {
  ...
  navigator.mediaSession.setActionHandler('seekto', (details) => {
    if (details.seekTime && Player.yt) Player.yt.seekTo(details.seekTime, true);
  });
  navigator.mediaSession.setActionHandler('seekbackward', (details) => {
    const cur = (Player.yt && Player.yt.getCurrentTime && Player.yt.getCurrentTime()) || 0;
    Player.yt && Player.yt.seekTo(Math.max(cur - (details.seekOffset || 10), 0), true);
  });
  navigator.mediaSession.setActionHandler('seekforward', (details) => {
    const cur = (Player.yt && Player.yt.getCurrentTime && Player.yt.getCurrentTime()) || 0;
    Player.yt && Player.yt.seekTo(cur + (details.seekOffset || 10), true);
  });
}
...
setInterval(() => {
  ...
  const mc = $('#mini-cur'); if (mc) mc.textContent = fmtTime(cur);
  const md = $('#mini-dur'); if (md) md.textContent = fmtTime(dur);
  if (!isPreviewing() && !seekDragging) {
    const nr = $('#np-range'); if (nr) nr.value = dur ? Math.round((cur / dur) * 1000) : 0;
    const nc = $('#np-cur'); if (nc) nc.textContent = fmtTime(cur);
    const nd = $('#np-dur'); if (nd) nd.textContent = fmtTime(dur);
  }
...
function viewLibrary(view, tab) {
  ...
  if (tab === 'favorites') {
    ...
    body = `<div class="lib-actions">
        ...
        <button class="pill-btn" id="btn-import">${icon('i-download')}<span>Import</span></button>
    ...
    body += cards
      ...
      : emptyHTML('No playlists yet', 'Use New playlist above to get started.', { ic: 'i-note' });
  }
  ...
  inp.onchange = () => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!d || (d.app !== 'imusic' && d.app !== 'rich-music' && d.app !== 'smw')) throw new Error('Not a valid backup file');
...
function openDeletePlaylist(pid) {
  ...
  body.innerHTML = `<div class="pl-form">
      ...
      <div class="pl-form-hint">Delete “${esc(pl.name)}”? This can’t be undone.</div>
      <div class="pl-form-actions">
...
async function openFloatWidget() {
  ...
  if (sysOk) {
    ...
    toast('Widget floating PiP aktif');
  } else if (docOk) {
    el.classList.add('hidden');
    toast('Widget window stays on top');
  } else {
...
/* ================= Background Keep-Alive Event Listeners ================= */
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    persistQueue();
    // Cegah YouTube IFrame auto-pause di latar belakang
    if (Player.yt && Player.ready && Player.current) {
    ...
    if (Player.yt && Player.ready && Player.current) {
      const st = Player.yt.getPlayerState && Player.yt.getPlayerState();
      if (st === YT.PlayerState.PAUSED && !document.body.classList.contains('paused')) {
