...
function restoreQueue() {
  ...
  document.title = `${s.title} • iMusiic`;
...
function backupLibrary() {
  ...
  a.href = url;
  a.download = `imusiic-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.rel = 'noopener';
  ...
  inp.onchange = () => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!d || (d.app !== 'imusiic' && d.app !== 'rich-music' && d.app !== 'smw')) throw new Error('Not an iMusiic backup');
